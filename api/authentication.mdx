...
| Scope | Use it for |
| --- | --- |
| `whatsapp:read` | Status, sync, conversations, contacts, messages, media, command status, and MCP WhatsApp read grants. |
| `whatsapp:write` | Pairing, sends, reactions, edits, deletes, read receipts, groups, unlink, and command enqueue. |
...
MCP clients do not use REST API keys. They authenticate with OAuth and receive a grant bound to a connection runtime.
