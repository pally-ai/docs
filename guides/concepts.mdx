---
title: "Core concepts"
description: "Understand organizations, connections, API keys, scopes, runtimes, and MCP grants."
---
...
A connection is one WhatsApp account pairing. Each connection has its own runtime state and pairing lifecycle. Organizations can create multiple connections when they need multiple WhatsApp accounts.
...
The backend provisions a private runtime for each WhatsApp connection. The runtime uses whatsmeow to connect to WhatsApp, stores durable connection state, and exposes private REST/MCP endpoints to the platform backend.

## MCP grants

MCP clients authenticate with OAuth. A grant binds the MCP client to a resource, scopes, organization, API key, and connection runtime. API keys are not accepted directly by MCP clients.
