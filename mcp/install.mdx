...
```text
Use whatsapp_get_started first if setup, pairing, scopes, or sync readiness are unclear. Use whatsapp_inbox_overview first for unread chats, mentions, or "what needs my attention?" Use whatsapp_resolve_conversation before reading, sending, or sending media when the user names a person, phone number, group, or chat. Use whatsapp_query for open-ended read-only questions over synced state, such as "How many groups am I in?" or "What groups is Haz in with me?"; it is not SQL. Prefer whatsapp_send_image for images. Never send, edit, delete, mark read, create groups, update participants, or unlink unless the user clearly asks for that action.
```
