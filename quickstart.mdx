...
Use the staging API while testing. Use production when your organization is ready.

```bash
export WHATSAPP_USE_BASE_URL="https://api.staging.whatsapp-use.com"
...
The response includes your organization, key metadata, runtime, and public endpoint paths.
...
When pairing succeeds, `paired` is `true`. `connected` is `true` when the runtime currently has an active WhatsApp socket.
